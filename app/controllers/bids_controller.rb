        ...
        if @bid.amount.present?
            if current_user.sufficient_funds?(@bid.amount, @auction_item) # ensure user has enough funds for bid
                if @bid.save
                    ...
                    redirect_to auction_items_path, notice: I18n.t("bids.create.success")
                else
                    redirect_to auction_items_path,
                                alert: "#{I18n.t('bids.create.bid_error')} #{@bid.errors.full_messages.join(', ')}"
                end
            else
                redirect_to auction_items_path, alert: I18n.t("bids.create.insufficient_funds")
            end
        else
            redirect_to auction_items_path, alert: I18n.t("bids.create.missing_amount")
        end
    end
