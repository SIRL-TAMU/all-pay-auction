...
module AuctionItemsHelper
  def auction_status(item)
    ...
    if item.opening_date > now
      return ['upcoming', "Opens on #{item.opening_date.strftime('%b %d, %Y')}"]
    elsif item.closing_date < now
      return ['closed', "Closed on #{item.closing_date.strftime('%b %d, %Y')}"]
    else
      ...
      return ['active', "Closing in #{days}d #{hours}h #{minutes}m"]
    end
  end
end
